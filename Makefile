...
test:
	RUST_BACKTRACE=1 cargo test --locked -- --test-threads=1
...
lint:
	cargo fmt --all -- --check
	cargo clippy --all-targets --all-features -- -D warnings -Dclippy::all -D clippy::pedantic -D clippy::cargo -A clippy::multiple_crate_versions
	cargo check
